...
parser.add_argument("--perform-jet-mixing", action='store_true')
    ...
    for sig_sample_id, sig_in_training_nums, mass in zip(signals, sig_in_training_nums_arr, masses):
        ...
        for xsec, sig_in_training_num in zip(xsecs, sig_in_training_nums):
            ...
            if how_much_to_inject == 0:
                ...
                inj_train_sample, inj_valid_sample, injected_signal = dapr.inject_signal(qcd_train_sample, sig_sample_ini, how_much_to_inject, train_split = 0.66)
                ...
                if args.perform_jet_mixing:
                    mixed_train_sample=dapr.perform_jet_mixing(inj_train_sample)
                    mixed_valid_sample=dapr.perform_jet_mixing(inj_valid_sample)
                else:
                    mixed_train_sample=inj_train_sample
                    mixed_valid_sample=inj_valid_sample
...
if args.save:
    qr_save_dir=os.path.join(experiment.model_dir_qr,f'delphesQCD_{methods[0]}')
    pathlib.Path(qr_save_dir).mkdir(exist_ok=True,parents=True)
    for k in range(0,params.kfold):
        disc_name=os.path.join(qr_save_dir,f'delphesQCD_fold_{k}_{methods[0]}ext.h5')
...
for k in range(0,params.kfold):
    for q,quantile in enumerate(quantiles):
        ...
        if inj == 0:
            csv_name = os.path.join(experiment.model_dir_qr,'delphes_models_lmfit_csv','delphes_bkg',f'smoothing_{srange[0]}_{srange[1]}_{methods[0]}',f'{quan}_{methods[1]}')
            if args.perform_jet_mixing:
                csv_name = os.path.join(experiment.model_dir_qr,'delphes_models_lmfit_csv','delphes_bkg_jet_mixing',f'smoothing_{srange[0]}_{srange[1]}_{methods[0]}',f'{quan}_{methods[1]}')
            ...
            csv_name = os.path.join(experiment.model_dir_qr,'delphes_models_lmfit_csv',f'{sample}_{inj}',f'smoothing_{srange[0]}_{srange[1]}_{methods[0]}',f'{quan}_{methods[1]}')
            if args.perform_jet_mixing:
                csv_name = os.path.join(experiment.model_dir_qr,'delphes_models_lmfit_csv',f'{sample}_{inj}_jet_mixing',f'smoothing_{srange[0]}_{srange[1]}_{methods[0]}',f'{quan}_{methods[1]}')
            subprocess.call('mkdir -p {}'.format(csv_name),shell=True)
            df.to_csv('{}/data_lmfit_modelresult_fold_{}_quant_q{:02}.csv'.format(csv_name,str(k),int(inv_quant*100)))

        cut_dict['{}_q{:02}'.format(str(k),int(inv_quant*100))] = y_mean

    if inj == 0:
        nosiginj_chunks[k].dump(result_paths.sample_file_path(params.qcd_sample_id, mkdir=True,overwrite=True,customname='delphes_bkg'),fold=k) # dumps individual chunks of background
    else:
...
if inj == 0:
    ...
    final_bkgsample.dump(result_paths.sample_file_path(params.qcd_sample_id, mkdir=True,overwrite=True,customname='delphes_bkg')) # Dumps entire background sample
else:
    ...
    for i,s in enumerate(signal_samples):
        s.dump(result_paths.sample_file_path(params.sig_sample_id,overwrite=True,mkdir=True,customname=f'signal_{params.sig_sample_id}'))
    # Dumps only injected signal files into single h5
