# Modified by: Aritra Bal, 01.07.2023
# To be used for training QR on Signal Region (SR) MC. 
# SR Data was reconstructed using VAE Run 50005 trained on QCD MC Sideband.  
...
if __name__ == "__main__":
    ...
    comments = [nAttempt,f'Train test split = {FRAC}', f'injected signal {sample} ={args.injection}' , 'QR trained on data in SR',f'smoothing range =[{bin_low},{bin_high}]',f'VAE Run = {args.run}',f'No. of folds = {nFolds}','Weighting: applied','mJJ>1460',f'batch_size = {BSIZE}',f'polynomial order = {pol_order}']
    ...
    for k in range(params.kfold):
        ...
        for sig_sample_id in signals:
            ...
            for xsec in xsecs:
                ...
                print("%%%%%%%%%%%%%%%%%%%%")
                qcd_train_sample.add_labels()
                sig_sample_ini.add_labels(signal=True)
                ...
                if not args.load:
                    ...
                    qr_save_dir=os.path.join(experiment.model_dir_qr,f'QR_models_fivesigma',load_attempt)
                    print(f"Attempting to load pre-trained discriminators from {qr_save_dir}")
                    discriminator.load(os.path.join(qr_save_dir,f'QCD_SRData_fold_{k}.h5'))
    ...
    if args.save:
        qr_save_dir=os.path.join(experiment.model_dir_qr,f'QR_models_fivesigma',comments[0],sample,str(inj))
        pathlib.Path(qr_save_dir).mkdir(exist_ok=True,parents=True)
        for k in range(0,params.kfold):
            disc_name=os.path.join(qr_save_dir,f'QCD_SRData_fold_{k}.h5')
    ...
    for k in range(0,params.kfold):
        for q,quantile in enumerate(quantiles):
            ...
            if inj == 0:
                csv_name = os.path.join(experiment.model_dir_qr,'models_lmfit_csv','QCD_5sigma',comments[0])
                ...
                csv_name = os.path.join(experiment.model_dir_qr,'models_lmfit_csv','QCD_5sigma',sample,f'{inj}',comments[0])
                subprocess.call('mkdir -p {}'.format(csv_name),shell=True)
                df.to_csv('{}/bkg_lmfit_modelresult_fold_{}_quant_q{:02}.csv'.format(csv_name,str(k),int(inv_quant*100)))
                write_log(csv_name,comments)
