...
if __name__ == "__main__":
        ...
        for sig_sample_id in signals:
            ...
            for xsec in xsecs:
                ...
                if not args.load:
                    ...
                    qr_save_dir=os.path.join(experiment.model_dir_qr,f'QR_models_MC',load_attempt)
                    print(f"Attempting to load pre-trained discriminators from {qr_save_dir}")
                    discriminator.load(os.path.join(qr_save_dir,f'QCD_MCOrig_fold_{k}.h5'))
    ...
    if args.save:
        qr_save_dir=os.path.join(experiment.model_dir_qr,f'QR_models_MC',comments[0])
        pathlib.Path(qr_save_dir).mkdir(exist_ok=True,parents=True)
        for k in range(0,params.kfold):
            disc_name=os.path.join(qr_save_dir,f'QCD_MCOrig_fold_{k}.h5')
