...
#signal_name = 'XToYYprimeTo4Q_MX3000_MY400_MYprime400_narrowReco' #'XToYYprimeTo4Q_MX2000_MY400_MYprime170_narrowReco' #'WkkToWRadionToWWW_M3000_Mr170Reco'
signals=['XToYYprimeTo4Q_MX3000_MY400_MYprime400_narrowReco','XToYYprimeTo4Q_MX3000_MY400_MYprime400_narrow_smear5Reco',\
         'XToYYprimeTo4Q_MX3000_MY400_MYprime400_narrow_smear10Reco','XToYYprimeTo4Q_MX3000_MY400_MYprime400_narrow_smear20Reco',\
            'XToYYprimeTo4Q_MX3000_MY400_MYprime400_narrow_smear30Reco']
signal_injections = [280.0,450.0,585.0,970.0,1170.0]
folds = 4
...
for signal_name,siginj in zip(signals,signal_injections):
    ...
    for k in tqdm.tqdm(range(folds)):
        ...
        for q in quantiles:
            if siginj == 0.:
                tmpdf = pd.read_csv(f'{case_qr_models}/run_{run_n}/models_lmfit_csv/QCD_MCOrig_asimov/{nAttempt}/bkg_lmfit_modelresult_fold_{k}_quant_q{q}.csv')
                sig_tmpdf = pd.read_csv(f'{case_qr_models}/run_{run_n}/models_lmfit_csv/QCD_MCOrig_asimov/{nAttempt}/sig_lmfit_modelresult_quant_q{q}.csv')
                ...
                tmpdf = pd.read_csv(f'/work/abal/CASE/QR_models/run_{run_n}/models_lmfit_csv/{signal_name}_{str(siginj)}/{nAttempt}/bkg_lmfit_modelresult_fold_{k}_quant_q{q}.csv')
                sig_tmpdf = pd.read_csv(f'/work/abal/CASE/QR_models/run_{run_n}/models_lmfit_csv/{signal_name}_{str(siginj)}/{nAttempt}/sig_lmfit_modelresult_quant_q{q}.csv')
                sig_p = np.poly1d(sig_tmpdf['par'].values.tolist()[::-1])
                sig_qrs.append(sig_p)
        ...
        ##### DATA-like ####
        if siginj>0.:
            with h5py.File(f"{case_qr_results}/sig_{signal_name}/xsec_0/loss_rk5_05/{nAttempt}/data_{signal_name}_{siginj}_fold_{k}.h5", "r") as data_f:
    ...
    case_qr_datasets=f'/ceph/abal/CASE/QR_datasets/run_{run_n}/wide_resonances/{nAttempt}'
    ...
    outfilename = f'bkg_QR_inj_{int(siginj)}'
    ...
    if siginj > 0.:
        ...
        dhf=h5py.File(os.path.join(case_qr_datasets,f'{outfilename}.h5'),'w')
        dhf.create_dataset('mjj', data=np.array(data_mjj))
        dhf.create_dataset('loss', data=np.array(data_loss))
        dhf.create_dataset('sel_q30', data=np.array(data_sel_q70))
        dhf.create_dataset('sel_q50', data=np.array(data_sel_q50))
        dhf.create_dataset('sel_q70', data=np.array(data_sel_q30))
        dhf.create_dataset('sel_q90', data=np.array(data_sel_q10))
        dhf.create_dataset('sel_q95', data=np.array(data_sel_q05))
        dhf.create_dataset('sel_q99', data=np.array(data_sel_q01))
        dhf.create_dataset('eventFeatures', data=np.array(data_event_features))
        dhf.create_dataset('eventFeatureNames', data=np.array(data_event_feature_names_reversed,dtype=dt))
        dhf.close()
